    ...
    public function widget( $args, $instance ) {
	    ...
	    if ( static::can_render( $instance ) && file_exists( $this->get_widget_template_path() ) ) {
		    ...
		    wp_enqueue_script(
			    ...
			    ES_PLUGIN_URL . $custom . 'locations.min.js',
			    array ( 'jquery' )
		    );

		    // Share plugin JS variables for locations script.
		    wp_localize_script( 'es-front-locations-script', 'Estatik', Estatik::register_js_variables() );
        }
    }
            ...
            case 'list':
                ...
                if ( ! empty( $field['values'] ) ) {
                    foreach ( $field['values'] as $value => $label ) {
                        ...
                        $content .= '<option value="' . esc_attr( $value ) . '" ' . $selected . '>' . $label . '</option>';
                    }
                }
